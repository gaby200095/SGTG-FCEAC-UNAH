...
const estadoColors = {
  aprobado:'#065f46', pendiente:'#92400e', rechazado:'#991b1b', en_proceso:'#1e3a8a'
};
...
const DashboardSecretaria = () => {
        ...
        if (user?.id_usuario){
          ...
          if (rUser.ok){
            ...
            if (u){
              ...
              setNombreSecretaria(`${n} ${a}`.trim() || (user.correo?.split('@')[0] || 'Secretaría General'));
            } else setNombreSecretaria(user.correo?.split('@')[0] || 'Secretaría General');
          }
        }
        const r = await call('/expedientes');
  ...
  return (
    <div className="reveal">
      <h2>Panel Secretaría General</h2>
      <p style={{ margin:'4px 0 6px', fontWeight:500 }}>
        Secretaría General: {nombreSecretaria || user?.correo}
      </p>
      <div style={{
        background:'#f1f5f9', padding:'10px 12px', borderRadius:6,
        fontSize:13, marginBottom:16, lineHeight:1.45
      }}>
        Como Secretaría General, usted tiene acceso completo a todos los expedientes.
      </div>
      <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom:12 }}>
        <div>
          <label style={{ fontSize:12, display:'block', marginBottom:4 }}>Estado:</label>
          <select value={filtroEstado} onChange={e=>setFiltroEstado(e.target.value)} style={{ padding:'4px 6px' }}>
                      ...
                      <tr key={row.id_expediente} style={{ borderBottom:'1px solid #e2e8f0' }}>
                        ...
                        <td style={td}>
                          <span style={{
                            ...
                            fontSize:12, borderRadius:12, textTransform:'capitalize',
